...
### 2. Dynamic Proxy (Castle.DynamicProxy/DispatchProxy)

| Aspect | Pros | Cons |
|--------|------|------|
| **Performance** | ⚠️ ~0.5-3μs overhead per call<br/>✅ Can be optimized with caching | ❌ Virtual method dispatch overhead |
| **Complexity** | ✅ No build-time tools<br/>✅ Works with existing DI | ❌ Requires DI registration changes<br/>❌ Interface-based or virtual methods only |
| **Flexibility** | ✅ Full runtime control<br/>✅ Dynamic enable/disable | ✅ Easy to modify behavior |
| **Cost** | ✅ Free (Castle/DispatchProxy) | |

### 3. Custom OpenTelemetry Sampler
...
Based on the requirements analysis, we recommend a **hybrid approach** combining Dynamic Proxy with Custom OpenTelemetry Sampler, enhanced with Orleans grain call context propagation:
...
graph TB
    ...
    subgraph "Method Tracing"
        K1[Method Call] --> L1[Dynamic Proxy Interceptor]
        L1 --> M1{Has TraceAttribute?}
